...
const addBranch = async (req, res) => {
    try {
      ...
      res.send({ success: false, message: 'Branch added successfully', docId: docRef.id });
    } catch (error) {
      console.error("Error adding document branch: ", error);
      res.status(500).json({ success: false, message: 'something went wrong when adding branch'});
    }
};
const getAllBranchs = async (req, res) => {
    ...
    const newList = list.sort((a, b) => a.maChiNhanh.localeCompare(b.maChiNhanh));
    res.json({success:true, branchs:newList});
    }
    catch(error){
        res.status(500).json({success:false, message: 'something went wrong when get data from ChiNhanh'})
        console.log(error);
        return [];
    }
};
...
const getBranchsBySearch = async (req, res) => {
  ...
  const myCollection = collection(firestore, 'ChiNhanh');
    ...
    const searchResults = list.filter((chiNhanh) => {
      ...
      const matchMaChiNhanh = maChiNhanh === '' || normalizeText(chiNhanh.maChiNhanh).includes(normalizeText(maChiNhanh));
      const matchTenChiNhanh = tenChiNhanh === '' || normalizeText(chiNhanh.tenChiNhanh).includes(normalizeText(tenChiNhanh));
      
      const matchSoLuongPhong =
      (slpDau == '' && slpCuoi == '') ||
      (slpDau != '' && parseFloat(chiNhanh.soLuongPhong) >= parseFloat(slpDau) && slpCuoi == '') ||
      (slpCuoi != '' && parseFloat(chiNhanh.soLuongPhong) <= parseFloat(slpCuoi)) && slpDau == ''||
      (slpDau != '' && slpCuoi != '' &&
    ...
    const sortList = searchResults.sort((a, b) => a.maChiNhanh.localeCompare(b.maChiNhanh));
...
module.exports={addBranch, getAllBranchs, updateBranch, deleteBranch, getBranchsBySearch}