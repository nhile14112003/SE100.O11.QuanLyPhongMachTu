...
const addFeedback = async (req, res) => {
    try {
      ...
      res.send({ success: false, message: 'Feedback added successfully', docId: docRef.id });
    } catch (error) {
      console.error("Error adding document feedback: ", error);
      res.status(500).json({ success: false, message: 'something went wrong when adding feedback'});
    }
};
